...
declare nodes=(115.146.85.196 115.146.85.120)
...
curl -X PUT "http://localhost:5984/_node/_local/_config/admins/${user}" --data "\"${pass}\""

curl -X PUT "http://${user}:${pass}@localhost:5984/_node/couchdb@${my_public_IP}/_config/chttpd/bind_address" --data '"0.0.0.0"'

curl ${my_public_IP}:5984

#curl -X PUT admin:password@localhost:5984/twitter
